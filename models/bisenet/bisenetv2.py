...
class SegmentHead(nn.Module):
    def __init__(self, in_chan, seghead_ratio, n_classes, sample_size, dropout_rate=0.1):
    ...
    def forward(self, x):
        ...
        feat = F.interpolate(feat, size=self.sample_size, mode="bilinear", align_corners=True)
        return feat


class BiSeNetV2(nn.Module):
    def __init__(self, n_classes=3, sample_size=(512, 1024), seghead_ratio=6):
        ...
        self.head = SegmentHead(128, seghead_ratio, n_classes, self.sample_size)
        if self.training:
            self.aux2 = SegmentHead(16, seghead_ratio, n_classes, self.sample_size)
            self.aux3 = SegmentHead(32, seghead_ratio, n_classes, self.sample_size)
            self.aux4 = SegmentHead(64, seghead_ratio, n_classes, self.sample_size)
            self.aux5_4 = SegmentHead(128, seghead_ratio, n_classes, self.sample_size)
